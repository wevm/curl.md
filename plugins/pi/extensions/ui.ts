...
export class SelectFilterList<item> extends Container {
  ...
  constructor(
    ...
    private readonly options: {
      emptyText?: string
      footerText?: string
      formatItem: (choice: item, props: { isSelected: boolean; theme: Theme }) => string
      placeholder: string
      searchText: (choice: item) => string
      title: string
    },
    private readonly onSelectCallback: (choice: item) => void,
...
export function dynamicBorder(colorFn: (s: string) => string) {
  return {
    invalidate() {},
    render(width: number) {
