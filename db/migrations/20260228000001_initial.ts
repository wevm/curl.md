...
export async function up(db: Kysely<unknown>): Promise<void> {
  ...
  await db.schema
    ...
    .addColumn('role', account_role(), (col) => col.notNull().defaultTo(sql`'user'::account_role`))
  ...
  await db.schema
    ...
    .addColumn('id', 'varchar(20)', (col) => col.primaryKey().defaultTo(nanoid()))
    .addColumn('account_id', 'varchar(20)', (col) => col.notNull().references('account.id'))
    ...
    .addColumn('created_at', 'timestamptz', (col) => col.notNull().defaultTo(now()))
    .addUniqueConstraint('unique_account_provider_provider', ['provider', 'provider_account_id'])
    .execute()
  ...
  await db.schema
    ...
    .addColumn('created_at', 'timestamptz', (col) => col.notNull().defaultTo(now()))
    .addUniqueConstraint('unique_organization_member', ['organization_id', 'account_id'])
    .execute()
  ...
  await db.schema.createIndex('index_request_hostname').on('request').column('hostname').execute()
  ...
  await db.schema.createIndex('index_account_login').on('account').column('login').execute()
